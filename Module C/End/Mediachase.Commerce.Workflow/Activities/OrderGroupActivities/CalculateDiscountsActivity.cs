    ...
    public class CalculateDiscountsActivity : OrderGroupActivityBase
    {
        ...
        private void AddGiftItemToAShipment(LineItem giftLineItem)
        {
            if (giftLineItem.Parent.Shipments.Count > 0)
            {
                ...
                shipment.AddLineItemIndex(giftLineItem.Parent.LineItems.Count - 1, giftLineItem.Quantity);
            }
        }
        ...
        private InventoryTrackingStatus GetInventoryStatus(Entry entry, string warehouseCode)
        {
            ...
            if (warehouse != null)
            {
                var inventory = WarehouseInventoryService.Service.Get(new CatalogKey(entry), warehouse);
                if (inventory != null)
                {
